...
export const addParticularDateExpenses = asyncHandler(async (req, res) => {
  ...
  await user.save();
  console.log(
    `${req.user.name} - Your ${totalExpenses} Expenses Added, current Pocket Money ${user.currentPocketMoney} `,
  );
...
export const showParticularDateExpenses = asyncHandler(async (req, res) => {
  ...
  if (!particularDateExpenses) {
    res.status(200).json(new ApiResponse(200, null, 'No Expenses Found !!'));
  }
  res
    .status(200)
...
export const showAllDateExpenses = asyncHandler(async (req, res) => {
  ...
  res.status(200).json(new ApiResponse(200, AllDateExpenses, 'All Expenses Found successfully!!'));
});
