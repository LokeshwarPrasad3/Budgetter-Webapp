...
export const sentTokenToResetPassword = asyncHandler(async (req, res) => {
  ...
  const token = jwt.sign({ _id: existedUser._id }, process.env.RESET_PASSWORD_TOKEN_SECRET, {
...
export const AddLentMoney = asyncHandler(async (req, res) => {
  ...
  const TotalLentMoney = user.LentMoneyHistory.reduce(
    (total, lent) => total + parseFloat(lent.price),
    0,
  );
  await user.save();
  res
    ...
    .json(
      new ApiResponse(
        201,
        { TotalLentMoney, currentPocketMoney: newPocketMoney },
