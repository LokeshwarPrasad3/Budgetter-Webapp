...
import { ChevronUp, Loader, MonitorSmartphone } from 'lucide-react';
    ...
    <>
      <div key={title} className="flex w-full items-center justify-center">
        {!open && (
          <motion.button
            ...
            className="flex h-8 w-full items-center justify-center border bg-gradient-to-tr from-green-800/80 via-green-700/80 to-green-600/80 px-3 text-center text-sm font-medium text-white outline-none hover:bg-gradient-to-r dark:bg-[#121212]"
          >
            <MonitorSmartphone className="mr-2 h-4 w-4" />
            <motion.p className="text-center" layoutId={`${title}-title`}>
              {title}
            </motion.p>
          </motion.button>
        )}
      ...
      <AnimatePresence>
        {open && (
          <motion.div
            layoutId={`${title}-wrapper`}
            className="absolute bottom-2 z-50 max-h-[96svh] w-full max-w-lg overflow-y-auto bg-muted p-1 shadow-[0_0_0_1px_rgba(0,0,0,0.08),0px_1px_2px_rgba(0,0,0,0.04)] outline-none"
            ref={ref}
            style={{ borderRadius: 10 }}
          >
            <motion.span
              aria-hidden
              className="absolute left-4 top-[17px] z-[-1] text-sm text-muted-foreground data-[success]:text-transparent"
